...
- 页面用法为，首先调用上述"export"
  地址生成excel，后台返回生成的excel文件id，前台使用该文件id下载生成好的excel文件,参考代码：web-flash\flash-vue-admin\src\views\system\cfg\cfg.js
...
值得注意的是，导出的excel文件会在存放在t_sys_file_info表中和对应的系统目录下，如果导出功能使用频率比较高的话，时间久了会生成很多文件信息。 那么就需要定时清理这些数据。

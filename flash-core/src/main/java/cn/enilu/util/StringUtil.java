...
public class StringUtil {
	...
	public static String strToDate(String str) throws ParseException {
		...
		if (StringUtil.isNotNull(str)) {
			...
			} else if(str.length()==5 || str.length()==2 || str.length()==3) {
				return str;
			} else if(str.length()==7) {
				sdf = new SimpleDateFormat("yyyyMMM", Locale.ENGLISH);
			} else if(str.length()==11){
				sdf = new SimpleDateFormat("dd-MMM-yyyy", Locale.ENGLISH);
			}
			Date date = sdf.parse(str);
			SimpleDateFormat djrcDate = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String format = djrcDate.format(date);
			return format;
		}else {
			return "";
		}

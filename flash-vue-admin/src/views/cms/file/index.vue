  ...
  <div class="app-container">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row
      ...
      <el-table-column label="缩略图">
        <template slot-scope="scope">
          <img style="width:60px" v-if="chkFileType(scope.row.originalFileName,'.jpg,.png,.gif,.bmp')"
               :src="imgHost+scope.row.id">
      ...
      <el-table-column label="下载">
        <template slot-scope="scope">
          <el-button type="text" icon="el-icon-download" size="mini"
                     @click.native="download(scope.row.id,scope.row.originalFileName)">下载
          </el-button>
        </template>
      </el-table-column>

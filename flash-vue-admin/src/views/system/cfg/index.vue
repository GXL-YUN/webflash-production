<template>
  <div class="app-container">
    <div class="block">
      ...
      <el-row>
        <el-col :span="24">
          <el-button type="success" size="mini" icon="el-icon-plus" @click.native="add" v-permission="['/cfg/add']">{{
            $t('button.add') }}
          </el-button>
          <el-button type="primary" size="mini" icon="el-icon-edit" @click.native="edit" v-permission="['/cfg/update']">
            {{ $t('button.edit') }}
          </el-button>
          <el-button type="danger" size="mini" icon="el-icon-delete" @click.native="remove"
                     v-permission="['/cfg/delete']">{{ $t('button.delete') }}
          </el-button>
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row
      ...
      <el-table-column :label="$t('config.value')">
        <template slot-scope="scope">
          <el-button type="text" v-if="scope.row.cfgValue.length>50" title="点击查看全部参数值"
                     @click="showCfgValDialog(scope.row)">{{scope.row.cfgValue.substr(0,25)}}
          </el-button>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" icon="el-icon-edit" @click.native="editItem(scope.row)"
                     v-permission="['/cfg/update']">{{ $t('button.edit') }}
          </el-button>

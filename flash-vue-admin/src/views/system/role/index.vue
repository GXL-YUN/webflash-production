<template>
  <div class="app-container">
    <div class="block">
      ...
      <el-row>
        <el-col :span="24">
          <el-button type="success" size="mini" icon="el-icon-plus" @click.native="add" v-permission="['/role/add']">{{
            $t('button.add') }}
          </el-button>
          <el-button type="primary" size="mini" icon="el-icon-edit" @click.native="edit" v-permission="['/role/edit']">
            {{ $t('button.edit') }}
          </el-button>
          <el-button type="danger" size="mini" icon="el-icon-delete" @click.native="remove"
                     v-permission="['/role/remove']">{{ $t('button.delete') }}
          </el-button>
          <el-button type="primary" size="mini" icon="el-icon-setting" @click.native="openPermissions"
                     v-permission="['/role/setAuthority']">权限配置
          </el-button>
        </el-col>
      </el-row>
    </div>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" icon="el-icon-edit" @click.native="editItem(scope.row)"
                     v-permission="['/role/edit']">{{ $t('button.edit') }}
          </el-button>
          <el-button type="text" size="mini" icon="el-icon-delete" @click.native="removeItem(scope.row)"
                     v-permission="['/role/remove']">{{ $t('button.delete') }}
          </el-button>
          <el-button type="text" size="mini" icon="el-icon-setting" @click.native="openPermissionsItem(scope.row)"
                     v-permission="['/role/setAuthority']">权限配置
          </el-button>
        </template>
      </el-table-column>

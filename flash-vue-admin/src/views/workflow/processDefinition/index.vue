<template>
  <div class="app-container">
    <div class="block">
      <el-row>
        <el-col :span="24">
          <el-button type="success" size="mini" icon="el-icon-plus" @click.native="onlineDrawingProcess"
                     v-permission="['/workflow/process/definition/edit']">{{ $t('button.add') }}
          </el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" icon="el-icon-edit" @click.native="onlineModificationProcess(scope.row)">{{
            $t('button.view') }}
          </el-button>
          <el-button type="text" size="mini" icon="el-icon-delete" @click.native="removeItem(scope.row)"
                     v-permission="['/workflow/process/definition/delete']">{{ $t('button.delete') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>

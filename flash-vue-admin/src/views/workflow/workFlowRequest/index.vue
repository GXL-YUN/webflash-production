<template>
  <div class="app-container">
    <div class="block">
      ...
      <el-row>
        <el-col :span="24">
          <el-button type="success" size="mini" icon="el-icon-caret-right" @click.native="add"
                     v-permission="['/workflow/request/add']">{{ $t('workflow.startProcessInstance') }}
          </el-button>
          <el-button type="primary" size="mini" icon="el-icon-edit" @click.native="edit"
                     v-permission="['/workflow/request/update']">{{ $t('button.edit') }}
          </el-button>
          <el-button type="danger" size="mini" icon="el-icon-delete" @click.native="remove"
                     v-permission="['/workflow/request/delete']">{{ $t('button.delete') }}
          </el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="Loading" border fit highlight-current-row
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" icon="el-icon-edit" @click.native="todo(scope.row)"
                     v-permission="['/workflow/request']">{{ $t('button.edit') }}
          </el-button>
          <el-button type="text" size="mini" icon="el-icon-view" @click.native="todo(scope.row)"
                     v-permission="['/workflow/request']">任务进度
          </el-button>
          <el-button type="text" size="mini" icon="el-icon-view" @click.native="todo(scope.row)"
                     v-permission="['/workflow/request']">{{ $t('button.view') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
